...
Program     : 167_msbf_m2_5_daily_remittance_exposure_validation_v0_2R5.sql
Version     : v0.2R5

Revision v0.2R5
----------------
Retains the v0.2R2 latest/archive `created_at` exclusion correction and expands
portfolio-summary reconciliation so positive validation uses the same count
and flow definitions as generation and final acceptance:

- start-of-day active count = `receivable_balance_before > 0`;
- end-of-day paid-off count = `paid_off_flag`;
- eligible POS sales, daily remittance, and cumulative remittance all reconcile.

Control inventory remains 120. No generated row, hash, policy, status, alert,
reason, source contract, or business methodology changes.
...
BEGIN
    ...
    (
        ...
        ((SELECT count(*) FROM msbf_m2.portfolio_daily_monitoring_summary AS p WHERE p.module1_run_id=(SELECT run_id FROM _m2_5_vctx) AND (p.opening_advance_count IS DISTINCT FROM (SELECT count(*) FROM msbf_m2.advance_daily_remittance_monitoring AS d WHERE d.module1_run_id=p.module1_run_id AND d.scenario_id=p.scenario_id AND d.monitoring_day_index=p.monitoring_day_index) OR p.active_advance_count IS DISTINCT FROM (SELECT count(*) FILTER(WHERE d.receivable_balance_before>0) FROM msbf_m2.advance_daily_remittance_monitoring AS d WHERE d.module1_run_id=p.module1_run_id AND d.scenario_id=p.scenario_id AND d.monitoring_day_index=p.monitoring_day_index) OR p.paid_off_count IS DISTINCT FROM (SELECT count(*) FILTER(WHERE d.paid_off_flag) FROM msbf_m2.advance_daily_remittance_monitoring AS d WHERE d.module1_run_id=p.module1_run_id AND d.scenario_id=p.scenario_id AND d.monitoring_day_index=p.monitoring_day_index))))::text,
        '0',
        coalesce(((SELECT count(*)=0 FROM msbf_m2.portfolio_daily_monitoring_summary AS p WHERE p.module1_run_id=(SELECT run_id FROM _m2_5_vctx) AND (p.opening_advance_count IS DISTINCT FROM (SELECT count(*) FROM msbf_m2.advance_daily_remittance_monitoring AS d WHERE d.module1_run_id=p.module1_run_id AND d.scenario_id=p.scenario_id AND d.monitoring_day_index=p.monitoring_day_index) OR p.active_advance_count IS DISTINCT FROM (SELECT count(*) FILTER(WHERE d.receivable_balance_before>0) FROM msbf_m2.advance_daily_remittance_monitoring AS d WHERE d.module1_run_id=p.module1_run_id AND d.scenario_id=p.scenario_id AND d.monitoring_day_index=p.monitoring_day_index) OR p.paid_off_count IS DISTINCT FROM (SELECT count(*) FILTER(WHERE d.paid_off_flag) FROM msbf_m2.advance_daily_remittance_monitoring AS d WHERE d.module1_run_id=p.module1_run_id AND d.scenario_id=p.scenario_id AND d.monitoring_day_index=p.monitoring_day_index)))), FALSE),
        'Portfolio opening, start-of-day active, and end-of-day paid-off counts reconcile to the daily ledger.'
    );
    PERFORM pg_temp.m2_5_add_check
    (
        'M2_5_POS_087_PORTFOLIO_REMITTANCE_RECONCILIATION',
        'PORTFOLIO_FLOW_ERRORS',
        ((SELECT count(*) FROM msbf_m2.portfolio_daily_monitoring_summary AS p WHERE p.module1_run_id=(SELECT run_id FROM _m2_5_vctx) AND (p.daily_eligible_pos_sales IS DISTINCT FROM (SELECT round(sum(d.source_eligible_pos_sales),2) FROM msbf_m2.advance_daily_remittance_monitoring AS d WHERE d.module1_run_id=p.module1_run_id AND d.scenario_id=p.scenario_id AND d.monitoring_day_index=p.monitoring_day_index) OR p.daily_remittance_amount IS DISTINCT FROM (SELECT round(sum(d.actual_remittance_amount),2) FROM msbf_m2.advance_daily_remittance_monitoring AS d WHERE d.module1_run_id=p.module1_run_id AND d.scenario_id=p.scenario_id AND d.monitoring_day_index=p.monitoring_day_index) OR p.cumulative_remittance_amount IS DISTINCT FROM (SELECT round(sum(d.cumulative_remittance_amount),2) FROM msbf_m2.advance_daily_remittance_monitoring AS d WHERE d.module1_run_id=p.module1_run_id AND d.scenario_id=p.scenario_id AND d.monitoring_day_index=p.monitoring_day_index))))::text,
        '0',
        ...
        'Portfolio eligible sales, daily remittance, and cumulative remittance reconcile to the daily ledger.'
    );
    PERFORM pg_temp.m2_5_add_check
    (
