...
Program     : 219_msbf_m2_11_detailed_report_v1.sql
Version     : v1
Work package: M2.11 Work Package 4
Correction  : M2_11_LIVE_EXECUTION_PROGRAM_219_CONTEXT_PROJECTION_CORRECTION_R1 / R13

Purpose
-------
Produce exactly twenty-four deterministically ordered, explicitly named,
read-only result sets covering lifecycle, source lineage, definitions,
candidate and strategy evidence, stress treatment, Pareto/frontier status,
governance-review priority, latest/archive reproduction, canonical hashes,
validation evidence, deterministic mismatches, and blocking/stage-boundary
violations.

Writes
------
No persistent writes. Reusable temporary reporting intermediates are
materialized once, indexed, and ANALYZE'd before downstream use.

Zero-row outputs
----------------
Result Sets 23 and 24 intentionally retain their column headers when zero rows
are returned.

Execution
---------
Execute after Program 217 and the completed Program 218 master-report export. Export each result set separately
using the result_set_code as the governed export identity. This source is
statically built and has not been parsed or executed by PostgreSQL in the
build environment.
============================================================================ */
...
/* ============================================================================
Section 1 — Accepted context and reusable reporting intermediates
============================================================================ */
CREATE TEMP TABLE tmp_report_m2_11_detail_context ON COMMIT DROP AS
SELECT
...
BEGIN
    ...
    FROM tmp_report_m2_11_detail_context;
    IF v_rows<>1 OR v_run_status<>'M2_11_ACCEPTED'
       OR v_contract_status<>'ACCEPTED' OR v_gate_status<>'PASS' THEN
      RAISE EXCEPTION
        'Program 219 requires one accepted M2.11 context; rows %, run %, contract %, gate %',
        v_rows,v_run_status,v_contract_status,v_gate_status;
    END IF;
END;
$m211_detail_context$;

CREATE TEMP TABLE tmp_report_m2_11_detail_canonical ON COMMIT DROP AS
WITH physical AS
(
 SELECT
   catalog_sequence,object_code,count(*)::bigint AS physical_row_count,
   md5
   (
     string_agg
     (
       ...
       ORDER BY
         CASE object_code
           WHEN 'msbf_m2.portfolio_strategy_summary'
             THEN split_part(business_key,'|',1)||'|'||split_part(business_key,'|',3)||'|'||split_part(business_key,'|',2)
           ...
           WHEN 'msbf_m2.portfolio_strategy_simulation_archive'
             THEN split_part(business_key,'|',1)||'|'||split_part(business_key,'|',2)||'|'||split_part(business_key,'|',4)||'|'||split_part(business_key,'|',3)
           ELSE business_key
         END
     )
   ) AS reconstructed_set_hash
 FROM msbf_m2.v_m2_11_canonical_entity_hash_source
 WHERE business_key=(SELECT run_id::text FROM tmp_report_m2_11_detail_context)
    OR business_key LIKE (SELECT run_id::text||'|%' FROM tmp_report_m2_11_detail_context)
...
(
 ...
 WHERE c.module1_run_id=(SELECT run_id FROM tmp_report_m2_11_detail_context)
...
CREATE UNIQUE INDEX tmp_report_m2_11_detail_canonical_u1
ON tmp_report_m2_11_detail_canonical(object_code);
ANALYZE tmp_report_m2_11_detail_canonical;

CREATE TEMP TABLE tmp_report_m2_11_detail_source ON COMMIT DROP AS
SELECT *
FROM tmp_report_m2_11_detail_canonical
WHERE catalog_sequence BETWEEN 6 AND 10;

CREATE UNIQUE INDEX tmp_report_m2_11_detail_source_u1
ON tmp_report_m2_11_detail_source(object_code);
ANALYZE tmp_report_m2_11_detail_source;

CREATE TEMP TABLE tmp_report_m2_11_detail_latest_archive ON COMMIT DROP AS
SELECT
...
WHERE coalesce(l.module1_run_id,a.module1_run_id)
      =(SELECT run_id FROM tmp_report_m2_11_detail_context);

CREATE UNIQUE INDEX tmp_report_m2_11_detail_latest_archive_u1
ON tmp_report_m2_11_detail_latest_archive(strategy_profile_code,reporting_scope_code);
ANALYZE tmp_report_m2_11_detail_latest_archive;

CREATE TEMP TABLE tmp_report_m2_11_detail_mismatch
(
    mismatch_family text NOT NULL,
...
INSERT INTO tmp_report_m2_11_detail_mismatch
(
    mismatch_family,object_code,business_key,observed_value,
    expected_value,mismatch_detail
)
SELECT
    'CANONICAL_COUNT_OR_HASH',object_code,object_code,
    coalesce(physical_row_count::text,'<MISSING>')||'|'||coalesce(reconstructed_set_hash,'<MISSING>'),
    coalesce(expected_row_count::text,'<MISSING>')||'|'||coalesce(registry_set_hash,'<MISSING>'),
    'Physical count or explicitly ordered set hash differs from the contract registry.'
FROM tmp_report_m2_11_detail_canonical
...
FROM tmp_report_m2_11_detail_latest_archive
WHERE reproduction_status<>'MATCH'
UNION ALL
SELECT
    'VALIDATION_EVIDENCE','msbf_ctl.run_evidence',evidence_code,
    status,'PASS','Persisted M2.11 evidence is not PASS.'
FROM msbf_ctl.run_evidence
WHERE run_id=(SELECT run_id FROM tmp_report_m2_11_detail_context)
  AND evidence_code LIKE 'M2_11_%'
  AND status<>'PASS'
UNION ALL
SELECT
    'STRESS_NONIMPROVEMENT','msbf_m2.application_portfolio_strategy_simulation',
    merchant_application_id||'|'||strategy_profile_code,
    'VIOLATION','NO_VIOLATION',
    'One or more stress-improvement violation flags are true.'
FROM msbf_m2.application_portfolio_strategy_simulation
WHERE module1_run_id=(SELECT run_id FROM tmp_report_m2_11_detail_context)
  AND
  (
    ...
    OR comparable_servicing_burden_improvement_violation_flag
  )
UNION ALL
SELECT
...
FROM tmp_report_m2_11_detail_context
WHERE combined_set_hash IS DISTINCT FROM
      (SELECT md5(string_agg(object_code||'|'||reconstructed_set_hash,
                             '|' ORDER BY catalog_sequence))
       FROM tmp_report_m2_11_detail_canonical);

CREATE INDEX tmp_report_m2_11_detail_mismatch_i1
ON tmp_report_m2_11_detail_mismatch(mismatch_family,object_code,business_key);
ANALYZE tmp_report_m2_11_detail_mismatch;

CREATE TEMP TABLE tmp_report_m2_11_detail_blocking
(
...
INSERT INTO tmp_report_m2_11_detail_blocking
(
    violation_family,object_name,business_key,violation_code,violation_detail
)
SELECT
    'BLOCKING_ERROR','msbf_ctl.profile_resolution_error',
    resolution_error_id::text,error_code,error_message
...
SELECT
    ...
    reason_code,'PROHIBITED_REASON_ACTION_FLAG',
...
SELECT
    ...
    reporting_scope_code,'MULTIPLE_PRIMARY_GOVERNANCE_PRIORITIES',
...
UNION ALL
SELECT
    'STAGE_BOUNDARY',table_schema||'.'||table_name,
    table_schema||'.'||table_name,'PREMATURE_M2_12_OBJECT',
...
CREATE INDEX tmp_report_m2_11_detail_blocking_i1
ON tmp_report_m2_11_detail_blocking(violation_family,object_name,business_key);
ANALYZE tmp_report_m2_11_detail_blocking;

/* ============================================================================
Section 2 — Exactly 24 ordered result sets
============================================================================ */
...
SELECT
    ...
    'FORMAL_ACCEPTANCE_IS_NOT_DEPLOYMENT_AUTHORIZATION'::text
      AS acceptance_interpretation,
...
FROM tmp_report_m2_11_detail_context c
...
SELECT
    ...
    'NO_DIRECT_M2_3_M2_5_M2_6_M2_8_M2_9_BUSINESS_READS'::text
      AS prohibited_direct_source_boundary
FROM tmp_report_m2_11_detail_context
ORDER BY run_id;
...
WHERE s.module1_run_id=(SELECT run_id FROM tmp_report_m2_11_detail_context)
...
WHERE r.module1_run_id=(SELECT run_id FROM tmp_report_m2_11_detail_context)
...
SELECT
    ...
    row_count_match_flag,registry_set_hash,reconstructed_set_hash,
    set_hash_match_flag
FROM tmp_report_m2_11_detail_source
ORDER BY catalog_sequence,object_code;
...
WHERE e.module1_run_id=(SELECT run_id FROM tmp_report_m2_11_detail_context)
...
WHERE v.module1_run_id=(SELECT run_id FROM tmp_report_m2_11_detail_context)
...
WHERE f.module1_run_id=(SELECT run_id FROM tmp_report_m2_11_detail_context)
ORDER BY
...
SELECT
    ...
    l.created_at
FROM msbf_m2.portfolio_strategy_simulation_latest l
WHERE l.module1_run_id=(SELECT run_id FROM tmp_report_m2_11_detail_context)
ORDER BY
...
FROM tmp_report_m2_11_detail_canonical x
CROSS JOIN tmp_report_m2_11_detail_context c
...
SELECT
    ...
    mismatch_detail
FROM tmp_report_m2_11_detail_mismatch
ORDER BY mismatch_family,object_code,business_key;

/* Result Set 24 — Blocking and stage-boundary violations */
SELECT
    'M2_11_DETAIL_24_BLOCKING_AND_STAGE_BOUNDARY_VIOLATIONS'::text
      AS result_set_code,
    violation_family,object_name,business_key,violation_code,violation_detail
FROM tmp_report_m2_11_detail_blocking
