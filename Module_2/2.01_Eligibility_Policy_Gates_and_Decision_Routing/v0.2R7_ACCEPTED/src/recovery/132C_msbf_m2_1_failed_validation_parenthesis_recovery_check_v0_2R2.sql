...
Program
132C_msbf_m2_1_failed_validation_parenthesis_recovery_check_v0_2R2.sql

Purpose
Confirm that Program 134 v0.2R1 committed successfully and that the failed
Program 135 parse did not alter the generated population, lifecycle, evidence,
or acceptance state.
...
(
    SELECT
        ...
        (
            SELECT count(*)
            FROM msbf_m2.application_eligibility_routing_comparison
            WHERE module1_run_id = (SELECT run_id FROM governed_run)
