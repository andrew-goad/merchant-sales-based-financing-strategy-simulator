...
Program     : 204_msbf_m2_10_schema_policy_portfolio_analytics_extension_v0_2R2.sql
Version     : v0.2R2
...
(
 SELECT
        ...
        v.kpi_code::text AS kpi_code,
        v.kpi_rank::integer AS kpi_rank,
        v.unit_code::text AS unit_code,
        v.zero_numeric::boolean AS zero_denominator_numeric_flag,
...
SELECT
    module1_run_id,kpi_code,kpi_rank,unit_code,
    zero_denominator_numeric_flag,description,definition_status,row_hash
FROM hashed
ON CONFLICT(module1_run_id,kpi_code) DO NOTHING;
...
(
 SELECT
        ctx.module1_run_id AS module1_run_id,
        v.code::text AS performance_tier_code,
        v.rank::integer AS performance_tier_rank,
        v.burden::numeric(12,6) AS burden_units,
        v.description::text AS description,
        'APPROVED'::text AS definition_status
...
(module1_run_id,performance_tier_code,performance_tier_rank,burden_units,
 description,definition_status,row_hash)
SELECT
    module1_run_id,performance_tier_code,performance_tier_rank,
    burden_units,description,definition_status,row_hash
FROM hashed
ON CONFLICT(module1_run_id,performance_tier_code) DO NOTHING;
...
(
 SELECT
        ctx.module1_run_id AS module1_run_id,
        v.code::text AS servicing_queue_code,
        v.rank::integer AS servicing_queue_rank,
        v.burden::numeric(12,6) AS burden_units,
        v.manual::boolean AS manual_review_flag,
        v.description::text AS description,
        'APPROVED'::text AS definition_status
...
SELECT
    module1_run_id,servicing_queue_code,servicing_queue_rank,
    burden_units,manual_review_flag,description,definition_status,row_hash
FROM hashed
ON CONFLICT(module1_run_id,servicing_queue_code) DO NOTHING;
...
(
 SELECT
        ctx.module1_run_id AS module1_run_id,
        v.code::text AS portfolio_analytics_reason_code,
        FALSE::boolean AS production_action_flag,
        v.description::text AS description,
        'APPROVED'::text AS definition_status
...
SELECT
    ...
    description,definition_status,row_hash
FROM hashed
ON CONFLICT(module1_run_id,portfolio_analytics_reason_code) DO NOTHING;
