...
Failed CTAS Projection Recovery Check
Version : v0.2R2
Purpose : Confirm that the v0.2R1 CREATE TEMP TABLE AS failure caused by a
          duplicate merchant_application_id projection committed no M1.9
          business data, evidence, or acceptance result.

Run only after cancelling the failed script 62 execution and issuing ROLLBACK.
...
FROM ctx c CROSS JOIN scenario_summary ss CROSS JOIN state s;
