/* ============================================================================
MSBF M1.5 Daily Deposit & Liquidity History — Acceptance Finalizer
Version : v0.2
Purpose : Resolve M1.5 from positive/negative evidence, canonical deposit-history
...
BEGIN
 ...
 IF v_status='M1_5_ACCEPTED' THEN RAISE EXCEPTION 'M1.5 is already accepted; acceptance result is frozen.'; END IF;
 IF v_status NOT IN ('M1_5_VALIDATED','M1_5_FAILED') THEN
  RAISE EXCEPTION 'M1.5 finalization requires completed validation; observed run_status=%.',v_status;
 END IF;

 SELECT COUNT(*),COUNT(*) FILTER(WHERE status='PASS') INTO v_positive_count,v_positive_pass
 FROM msbf_ctl.run_evidence WHERE run_id=v_run_id AND evidence_code ~ '^M1_5_POS_[0-9]{2}_';
 SELECT COUNT(*),COUNT(*) FILTER(WHERE status='PASS') INTO v_negative_count,v_negative_pass
 FROM msbf_ctl.run_evidence WHERE run_id=v_run_id AND evidence_code LIKE 'M1_5_NEG_%';
 SELECT COUNT(*) INTO v_failed FROM msbf_ctl.run_evidence
 WHERE run_id=v_run_id AND (evidence_code ~ '^M1_5_POS_[0-9]{2}_' OR evidence_code LIKE 'M1_5_NEG_%') AND status='FAIL';
 ...
 FROM msbf_m1.m1_5_expected_deposit_snapshot(v_run_id);
 SELECT md5(string_agg(entity_key||'|'||row_hash,'||' ORDER BY entity_key)) INTO v_actual_hash
 FROM msbf_m1.m1_5_actual_deposit_snapshot(v_run_id);
 SELECT metric_value_text INTO v_stored_hash FROM msbf_ctl.run_evidence
 WHERE run_id=v_run_id AND evidence_code='M1_5_DEPOSIT_SET_HASH' AND segment_key='PORTFOLIO';

 SELECT COUNT(*),COUNT(DISTINCT merchant_id),COUNT(DISTINCT observation_date)
 INTO v_rows,v_merchants,v_dates FROM msbf_m1.merchant_deposit_daily_base WHERE generated_by_run_id=v_run_id;
 ...
 v_result:=CASE WHEN v_positive_count=56 AND v_positive_pass=56
 ...
 v_observed:=jsonb_build_object(
  ...
  'deposit_rows',v_rows,'merchants',v_merchants,'dates',v_dates,
  'deposit_set_hash',v_actual_hash,'downstream_rows',v_downstream,'blocking_errors',v_errors)::text;

 SELECT COALESCE(MAX(review_version),0)+1 INTO v_review_version
 FROM msbf_ctl.acceptance_gate_result WHERE run_id=v_run_id AND gate_id='M1_5_DAILY_DEPOSIT_LIQUIDITY';
 ...
 VALUES(v_run_id,'M1_5_DAILY_DEPOSIT_LIQUIDITY',v_review_version,v_result,v_observed,
  '{"positive_checks":56,"negative_controls":4,"row_mismatches":0,"deposit_rows":135000,"merchants":750,"dates":180,"downstream_rows":0,"blocking_errors":0}',
  CASE WHEN v_result='PASS' THEN 'M1.5 deterministic daily deposit and liquidity history passed all acceptance criteria.'
       ELSE 'M1.5 failed one or more daily deposit and liquidity-history acceptance criteria.' END,
  'Synthetic latent operating-account history only. This gate does not validate real bank statements, transaction categorization, calibrated liquidity behavior, source observability, scenario forecasts, credit risk, pricing, servicing, accounting, capital, fair lending, compliance, or production use.',
 ...
 VALUES(v_run_id,'M1_5_ACCEPTANCE_SUMMARY','PORTFOLIO','M1.5 daily deposit and liquidity acceptance',v_observed,'JSON_TEXT',v_result,
  CASE WHEN v_result='PASS' THEN 'M1.5 accepted; M1.6 matched scenario overlay development is authorized.'
       ELSE 'M1.5 failed; subsequent analytical generation remains prohibited.' END)
 ...
 IF v_result='PASS' THEN
  UPDATE msbf_ctl.run_registry SET run_status='M1_5_ACCEPTED',
   notes='M1.5 daily deposit and liquidity history accepted. Authorized for M1.6 matched scenario overlay generation.'
  WHERE run_id=v_run_id;
 ELSE
  UPDATE msbf_ctl.run_registry SET run_status='M1_5_FAILED',
   notes='M1.5 acceptance failed; subsequent analytical generation prohibited.'
  WHERE run_id=v_run_id;
 END IF;
END $do$;
...
LEFT JOIN LATERAL(
 SELECT * FROM msbf_ctl.acceptance_gate_result x WHERE x.run_id=r.run_id AND x.gate_id='M1_5_DAILY_DEPOSIT_LIQUIDITY' ORDER BY review_version DESC LIMIT 1
) g ON true
