/* ============================================================================
MSBF M1.4 Enterprise Merchant Ecosystem — Negative Controls
Version : v0.2
Purpose : Demonstrate fail-closed behavior for missing parameters, invalid POS
          source readiness, history-window drift, and prohibited regeneration.
============================================================================ */
BEGIN;
...
BEGIN
 ...
 IF v_status NOT IN ('M1_4_VALIDATED','M1_4_FAILED') THEN
   RAISE EXCEPTION 'M1.4 negative controls require completed positive validation; observed run_status=%.',v_status;
 END IF;
 DELETE FROM msbf_ctl.run_evidence WHERE run_id=v_run_id AND evidence_code LIKE 'M1_4_NEG_%';
END $do$;

CREATE TEMP TABLE _m1_4_negative_results(
...
BEGIN
 ...
 BEGIN
   UPDATE msbf_ctl.run_registry SET run_status='M1_3_ACCEPTED' WHERE run_id=v_run_id;
   DELETE FROM msbf_ctl.run_parameter_snapshot
   WHERE run_id=v_run_id AND parameter_name='industry_daily_sales_center' AND scope_key='INDUSTRY:GENERAL_RETAIL';
   PERFORM msbf_m1.m1_4_assert_generation_ready(v_run_id);
   RAISE EXCEPTION 'CONTROL_NOT_REJECTED';
 EXCEPTION WHEN OTHERS THEN
   v_msg:=SQLERRM;
   v_pass:=position('requires 86 resolved parameter/scope pairs' in SQLERRM)>0;
 END;
 INSERT INTO _m1_4_negative_results VALUES(
  'M1_4_NEG_01_MISSING_PARAMETER_REJECTED','Missing required POS parameter',v_msg,
  'Generation rejected because fewer than 86 required parameter/scope pairs resolve',v_pass,
  'The generation gate fails closed when one governed industry parameter is missing.');
END $do$;
...
BEGIN
 ...
 BEGIN
   ...
   WHERE run_id=v_run_id AND source_code='POS_DAILY';
   PERFORM msbf_m1.m1_4_assert_generation_ready(v_run_id);
   RAISE EXCEPTION 'CONTROL_NOT_REJECTED';
 EXCEPTION WHEN OTHERS THEN
   v_msg:=SQLERRM;
   v_pass:=position('requires exactly one approved, contract-ready POS_DAILY source snapshot' in SQLERRM)>0;
 END;
 INSERT INTO _m1_4_negative_results VALUES(
  'M1_4_NEG_02_SOURCE_NOT_READY_REJECTED','Invalid POS source readiness',v_msg,
  'Generation rejected because POS_DAILY is not contract-ready',v_pass,
  'The source gate fails closed when the daily POS source is not ready.');
END $do$;
...
BEGIN
 ...
 EXCEPTION WHEN OTHERS THEN
   v_msg:=SQLERRM;
   v_pass:=position('requires 750 merchants, 750 applications, and 180 days' in SQLERRM)>0;
 END;
 INSERT INTO _m1_4_negative_results VALUES(
  'M1_4_NEG_03_HISTORY_DRIFT_REJECTED','History-window drift',v_msg,
...
BEGIN
 ...
 EXCEPTION WHEN OTHERS THEN
   v_msg:=SQLERRM;
   v_pass:=position('requires run_status=M1_3_ACCEPTED' in SQLERRM)>0
           OR position('regeneration is prohibited' in SQLERRM)>0;
 END;
 INSERT INTO _m1_4_negative_results VALUES(
  'M1_4_NEG_04_REGENERATION_REJECTED','Post-generation rerun rejection',v_msg,
  'Generation rejected after baseline history already exists',v_pass,
  'Accepted upstream state and persisted M1.4 rows cannot be silently regenerated.');
END $do$;
...
WHERE run_id=(SELECT run_id FROM msbf_ctl.run_registry WHERE run_code='M1_V0_2_BASELINE_BUILD' AND run_version=1)
  AND evidence_code LIKE 'M1_4_NEG_%'
